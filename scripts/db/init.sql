...
CREATE TABLE IF NOT EXISTS registros_combustible (
  ...
  foto_odometro_antes VARCHAR(500) DEFAULT NULL,
  foto_odometro_despues VARCHAR(500) DEFAULT NULL,
  foto_factura VARCHAR(500) DEFAULT NULL,
  foto_voucher VARCHAR(500) DEFAULT NULL,
...
CREATE INDEX IF NOT EXISTS idx_registros_user_id ON registros_combustible(user_id);
CREATE INDEX IF NOT EXISTS idx_registros_fecha ON registros_combustible(fecha_creacion);
CREATE INDEX IF NOT EXISTS idx_registros_sincronizado ON registros_combustible(sincronizado);

INSERT IGNORE INTO _migrations (name) VALUES ('001_create_tables.sql');

-- Seed usuarios
INSERT INTO usuarios (username, nombre, password, role) VALUES
  ('admin', 'Administrador', 'admin2024', 'admin'),
  ('conductor1', 'Carlos Mejía', 'flota2024', 'conductor'),
  ('conductor2', 'Ana López', 'flota2024', 'conductor')
