...
export async function POST({ request }: { request: Request }) {
  try {
    ...
    const result = await query<any>('INSERT INTO tipos_combustible (nombre) VALUES (?)', [nombre]);
    ...
    if (err.code === 'ER_DUP_ENTRY') {
      return new Response(JSON.stringify({ error: 'El tipo de combustible ya existe' }), {
        status: 409,
        headers: { 'Content-Type': 'application/json' },
      });
    }
...
export async function PUT({ request }: { request: Request }) {
  try {
    ...
    await query('UPDATE tipos_combustible SET nombre = ? WHERE id = ?', [nombre, id]);
...
export async function DELETE({ request }: { request: Request }) {
  try {
    ...
    await query('DELETE FROM tipos_combustible WHERE id = ?', [id]);
    return new Response(JSON.stringify({ success: true }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
